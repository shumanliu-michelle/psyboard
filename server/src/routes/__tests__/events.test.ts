...
import { BACKLOG_COLUMN_ID, DONE_COLUMN_ID } from '../../types.js'
...
describe('broadcast() — tabId behavior', () => {
  ...
  it('broadcast includes {source:"tab",updated} when a task is reordered (including move out of Done)', (done) => {
    const sseReq = request(app).get('/api/events').buffer(true)
    setTimeout(async () => {
      // Create a task in Done column
      const create = await request(app).post('/api/tasks').send({ title: 'Done task', columnId: DONE_COLUMN_ID })
      const taskId = create.body.id
      setTimeout(() => {
        // Move it out of Done to Backlog
        request(app)
          .post('/api/tasks/reorder')
          .set('X-Tab-Id', 'my-tab')
          .send({ taskId, targetColumnId: BACKLOG_COLUMN_ID, newIndex: 0 })
          .end(() => {
            sseReq.end((_err, res) => {
              expect(res.text).toMatch(/"source":"tab"/)
              expect(res.text).toMatch(/"updated"/)
